...
datacsv <- read_csv("C:/datos/educalab/POC_PROGRAMACION.csv")
View(datacsv)
...
for(i in 1:nrow(data2) )
    {       # for-loop over rows
 # data2[i, ] <- data2[i, ] - 100
  dataci = data2[i, 2]$`Número Documento`
  datanombre=data2[i, 3]$`Apellidos / Nombres`
...
persona <- data.frame()
persona <- data.frame(i,razonSocial,ruc,nombreComercial,claseContribuyente,estadoContribuyente,
                      fechaActualizacion,
...
write_csv(personasdf, ("C:/datos/educalab/POC_PROGRAMACION_PERSONAS.csv"))
